...
uint8 ADC_Cal(ADC_MemMapPtr adcmap)
{
  ...
  while ( (ADC_SC1_REG(adcmap,_A) & ADC_SC1_COCO_MASK ) == COCO_NOT ); // Wait calibration end
...
void ADC_Config_Alt(ADC_MemMapPtr adcmap, tADC_ConfigPtr ADC_CfgPtr)
{
 ...
 ADC_SC1_REG(adcmap,_A)= ADC_CfgPtr->STATUS1A;       
 ADC_SC1_REG(adcmap,_B)= ADC_CfgPtr->STATUS1B;
}
