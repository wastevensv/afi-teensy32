...
#if ((((FMSTR_USE_SCI)?1:0) + ((FMSTR_USE_JTAG)?1:0) + ((FMSTR_USE_CAN)?1:0) + ((FMSTR_USE_MQX_IO)?1:0) + ((FMSTR_DISABLE)?1:0) + ((FMSTR_USE_USB_CDC)?1:0) + ((FMSTR_USE_PDBDM)?1:0)) > 1)
#error More than one communication interface selected for FreeMASTER driver
#endif

/* All Interface options are set to 0 */
#if ((((FMSTR_USE_SCI)?1:0) + ((FMSTR_USE_JTAG)?1:0) + ((FMSTR_USE_CAN)?1:0) + ((FMSTR_USE_MQX_IO)?1:0) + ((FMSTR_DISABLE)?1:0) + ((FMSTR_USE_USB_CDC)?1:0) + ((FMSTR_USE_PDBDM)?1:0)) == 0)
#error All Interface options are set to 0, please chooce one intercace or set FMSTR_DISABLE macro to 1.
#endif
...
/* CAN is MSCAN or FLEXCAN */
    ...
    #error Cannot use FlecCan/MsCan and define macro FMSTR_USE_CAN to 0
    #endif
...
/* MQX IO communication interface warnings and errors */
...
#if FMSTR_USE_SCI || FMSTR_USE_JTAG || FMSTR_USE_USB_CDC || FMSTR_USE_MQX_IO
    ...
    /* Packet Driven BDM communication does not support any interrupt mode */
         ...
         #warning "FreeMASTER Packet Driven BDM driver doesn't support any Interrupt mode"
    #endif
#endif
