...
#  go here.
#OBJ_FILES += $(OBJDIR)/crt0.o
OBJ_FILES += $(OBJDIR)/pins_teensy.o
#OBJ_FILES += $(OBJDIR)/yield.o
OBJ_FILES += $(OBJDIR)/EventResponder.o
OBJ_FILES += $(OBJDIR)/analog.o
OBJ_FILES += $(OBJDIR)/mk20dx128.o
...
hw_all: $(OUTPUTDIR)/$(PROJECT).hex $(OUTPUTDIR)/$(PROJECT).bin stats dump

hw/$(TARGET)/hal.a: $(OBJ_FILES)
	$(AR) $(ARFLAGS) ./$@ $^
...
#  Linker invocation
$(OUTPUTDIR)/$(PROJECT).elf: $(OBJ_FILES) afi.a
	@mkdir -p $(dir $@)
